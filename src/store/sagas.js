...
function* api_v1_useres_listWorker(action) {
  try {
    const result = yield call(apiService.api_v1_useres_list, action)
    yield put(actions.api_v1_useres_listSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_useres_listFailed(err, action))
  }
}
function* api_v1_useres_listWatcher() {
  yield takeEvery(types.API_V1_USERES_LIST, api_v1_useres_listWorker)
}
function* api_v1_useres_createWorker(action) {
  try {
    const result = yield call(apiService.api_v1_useres_create, action)
    yield put(actions.api_v1_useres_createSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_useres_createFailed(err, action))
  }
}
function* api_v1_useres_createWatcher() {
  yield takeEvery(types.API_V1_USERES_CREATE, api_v1_useres_createWorker)
}
function* api_v1_useres_readWorker(action) {
  try {
    const result = yield call(apiService.api_v1_useres_read, action)
    yield put(actions.api_v1_useres_readSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_useres_readFailed(err, action))
  }
}
function* api_v1_useres_readWatcher() {
  yield takeEvery(types.API_V1_USERES_READ, api_v1_useres_readWorker)
}
function* api_v1_useres_updateWorker(action) {
  try {
    const result = yield call(apiService.api_v1_useres_update, action)
    yield put(actions.api_v1_useres_updateSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_useres_updateFailed(err, action))
  }
}
function* api_v1_useres_updateWatcher() {
  yield takeEvery(types.API_V1_USERES_UPDATE, api_v1_useres_updateWorker)
}
function* api_v1_useres_partial_updateWorker(action) {
  try {
    const result = yield call(apiService.api_v1_useres_partial_update, action)
    yield put(actions.api_v1_useres_partial_updateSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_useres_partial_updateFailed(err, action))
  }
}
function* api_v1_useres_partial_updateWatcher() {
  yield takeEvery(
    types.API_V1_USERES_PARTIAL_UPDATE,
    api_v1_useres_partial_updateWorker
  )
}
function* api_v1_useres_deleteWorker(action) {
  try {
    const result = yield call(apiService.api_v1_useres_delete, action)
    yield put(actions.api_v1_useres_deleteSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_useres_deleteFailed(err, action))
  }
}
function* api_v1_useres_deleteWatcher() {
  yield takeEvery(types.API_V1_USERES_DELETE, api_v1_useres_deleteWorker)
}
...
export default function* rootSaga() {
  const sagas = [
    ...
    api_v1_useres_listWatcher,
    api_v1_useres_createWatcher,
    api_v1_useres_readWatcher,
    api_v1_useres_updateWatcher,
    api_v1_useres_partial_updateWatcher,
    api_v1_useres_deleteWatcher,
